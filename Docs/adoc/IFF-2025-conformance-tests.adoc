...
:notitle:
// -- Document metadata --
:description: Complete conformance test matrix for IFF-2025 parser and generator implementations.
:keywords: IFF, IFF-2025, conformance, tests, validation
:copyright: © 2025 Claudio Russo and Dario Russo — MIT License

ifdef::backend-pdf[]
<<<
[.text-center]
[.big]*IFF-2025 Conformance Test Matrix*
...
ifndef::backend-pdf[]
[.lead]
*IFF-2025 Conformance Test Matrix* — _Version 1.0_
endif::[]

This document defines the complete set of conformance tests for IFF-2025 parser and generator implementations.
Tests are numbered globally and organised by functional area.
...
== Bootstrapping and Mode Selection

=== IFF-85 Auto-Detection
...
|R8  |iff2025_header_default_flags |`' IFF'` with all-zero flags produces identical behaviour to IFF-85 defaults, but `iff85_locked == 0`. Directives are accepted.
|R9  |iff2025_version_zero_locks_85 |`' IFF'` with version=0 activates IFF-85 mode. `iff85_locked == 1`.
|R10 |iff2025_unknown_version_fails |`' IFF'` with version=99. Parse fails (unrecognised version).
|===


== Header Flag Processing
...
== Scope Guards (Mid-Stream `' IFF'` Rejection)
...
== Multiple Top-Level Forms and EOF
...
|R108 |flags_all_defaults_explicit |`' IFF'` with every flag field explicitly set to its default (0). Identical behaviour to IFF-85. `iff85_locked == 0`.
|===
...
== Generator Initialisation and Memory Mode
...
== Container Lifecycle — Blobbed Mode
...
== Container Lifecycle — Progressive Mode
...
== Header Flag Effects on Generation
...
The original 26 tests cover 35 conformance points (9 tests verify two aspects each).
These multi-coverage mappings are marked below.

.Original Test to Conformance Point Mapping
